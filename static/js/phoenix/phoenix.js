(function(global, factory) {
	...
	const appCalendarInit = () => {
		...
		(() => {
			const e = new Date,
				...
				n = e.getDay(),
				l = `${a}  ${t},  ${e.getFullYear()}`;
			document.querySelector(r) && (document.querySelector(r).textContent = (e => ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"][e])(n)), document.querySelector(o) && (document.querySelector(o).textContent = l);
		})();
		const S = e => {
				const {
					currentViewType: t
		...
		if (f) {
			...
			S(e.currentData), document.querySelectorAll(c).forEach((t => {
				...
				e.addEvent({
					...
					description: l.value
				}), t.target.reset(), window.bootstrap.Modal.getInstance(w).hide();
			})), w && w.addEventListener(v, (({
				currentTarget: e
			}) => {
	...
	const echartSetOption = (e, t, o) => {
		...
		e.setOption(merge$2(o(), t)), a.addEventListener("clickControl", (({
			detail: {
				control: a
			}
		}) => {
	...
	const chatInit = () => {
		const {
			getData: e
		} = window.phoenix.utils, t = ".chat-sidebar", a = ".chat-textarea", o = ".btn-emoji", r = "[data-chat-thread]", c = "[data-chat-thread-tab]", n = "[data-chat-thread-tab-content]", s = document.querySelector(t), i = document.querySelector(a), d = document.querySelector(o), l = document.querySelectorAll(r), u = document.querySelector(c), m = document.querySelector(n);
		if (u) {
		...
		if (l.forEach((e => {
			...
			})), i && i.setAttribute("placeholder", "Type your message..."), d) {
		...
		listInit = () => {
			...
			if (window.List) {
				...
				t.length && t.forEach((t => {
					...
					const p = () => {
						c && (c.innerHTML = `${d.i} to ${m} <span class='text-600'> Items of </span>${g}`), l && togglePaginationButtonDisable(l, 1 === v), a && togglePaginationButtonDisable(a, v === b), v > 1 && v < b && (togglePaginationButtonDisable(a, !1), togglePaginationButtonDisable(l, !1));
					};
					p(), a && a.addEventListener("click", (e => {
						e.preventDefault(), v += 1;
						const t = d.i + u;
						t <= d.size() && d.show(t, u), m += d.visibleItems.length, p();
					})), l && l.addEventListener("click", (e => {
						e.preventDefault(), v -= 1, m -= d.visibleItems.length;
						const t = d.i - u;
						t > 0 && d.show(t, u), p();
					}));
					const L = () => {
					...
					if (s && s.addEventListener("click", (() => {
							console.log({
								items: d.items
							}), d.show(1, g), b = 1, v = 1, m = g, p(), L();
						})), o && o.addEventListener("click", (() => {
							d.show(1, u), b = Math.ceil(g / u), v = 1, m = d.visibleItems.length, p(), L();
	...
	const handleNavbarVerticalCollapsed = () => {
		const {
			...
			hasClass: a,
			setItemToStore: t
		} = window.phoenix.utils, l = "html", r = ".navbar-vertical-toggle", o = ".navbar-vertical .navbar-collapse", s = "click", c = "mouseover", n = "mouseleave", v = "navbar.vertical.toggle", d = "navbar-vertical-collapsed", i = "navbar-vertical-collapsed-hover", u = document.querySelector(r), p = document.querySelector(l), b = document.querySelector(o);
	...
	const {
		...
		const {
			setItemToStore: e,
			getData: t
		} = window.phoenix.utils, o = new DomNode(document.body), a = document.querySelector(".navbar-vertical"), r = document.querySelector(".navbar-top");
		initialDomSetup(o.node), o.on("click", (c => {
			...
			if (n.data("theme-control")) {
				...
				switch ("phoenixTheme" === l && "boolean" == typeof i && (i = i ? "dark" : "light"), config.hasOwnProperty(l) && e(l, i), window.history.replaceState(null, null, window.location.pathname), l) {
					case "phoenixTheme": {
						...
						c.currentTarget.dispatchEvent(e), changeTheme(o.node);
						break
					}
					case "phoenixNavbarVerticalStyle":
						a.classList.remove("navbar-darker"), "default" !== i && a.classList.add(`navbar-${i}`);
						break;
					case "phoenixNavbarTopStyle":
						r.classList.remove("navbar-darker"), "transparent" !== i && r.classList.add(`navbar-${i}`);
						break;
					...
					break;
					case "phoenixIsRTL":
						localStorage.setItem("phoenixIsRTL", n.node.checked), window.location.reload();
						break;
					case "reset":
		...
		clickBulkCheckbox() {
			this.element.addEventListener("click", (() => {
				...
				this.toggleDisplay(), this.bulkSelectRows.forEach((e => {
					const t = new DomNode(e);
					t.checked = this.element.checked, t.setAttribute("checked", this.element.checked);
				}));
			}));
		}
		clickRowCheckbox() {
